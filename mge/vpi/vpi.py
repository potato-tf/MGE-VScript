...
from vpi_config import SECRET

LOGGER = vpi_interfaces.LOGGER

LOGGER.info("VScript-Python Interface")
...
DB_DATABASE	   = genv("DB_DATABASE", "")
DB_PASSWORD	   = genv("DB_PASSWORD", None)
SCRIPTDATA_DIR = genv("SCRIPTDATA_DIR", "")
...
async def main():
	global POOL
	POOL = await aiomysql.create_pool(host=DB_HOST, user=DB_USER, password=DB_PASSWORD, port=DB_PORT, db=DB_DATABASE, loop=loop, autocommit=False)
	...
	while True:
		...
		if (last_modtime != last_interface_modtime):
			...
			try:
				...
				LOGGER.error("INVALID INTERFACE MODULE CODE!")
