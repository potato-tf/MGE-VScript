...
if (ELO_TRACKING_MODE > 1)
{
	...
	VPI.AsyncCall({
		...
		func = "VPI_MGE_DBInit",
		callback = function(response, error) {
			printl(MGE_Localization[DEFAULT_LANGUAGE][error ? "VPI_DBInitSuccess" : "VPI_DBInitError"] + error)
		}
	})
}
